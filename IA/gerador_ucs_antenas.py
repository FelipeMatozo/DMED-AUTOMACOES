...
def encontrar_antenas_mais_proximas(uc_lat, uc_lon, n=5, threshold_dist=0.008, offset=0.00002):
    ...
    colunas = ["latitude_antena", "longitude_antena", "operadora", "estacao", "tecnologia", "faixa"]
    antenas = buscar_todos_dados("parana_antenas", colunas)

    distancias_antenas = []
    
    # Calcular as distâncias entre a UC e as antenas
    for antena_lat, antena_lon, operadora, estacao, tecnologia, faixa in antenas:
        distancia = geodesic((uc_lat, uc_lon), (antena_lat, antena_lon)).kilometers
        distancias_antenas.append((distancia, (antena_lat, antena_lon, operadora, distancia, estacao, tecnologia, faixa)))
    ...
    for i in range(1, len(antenas_mais_proximas)):
        ...
        if distancia_entre_antenas < threshold_dist:
            lat_atual = float(lat_atual)  # Garantir que seja float
            lon_atual = float(lon_atual)  # Garantir que seja float
            antenas_mais_proximas[i] = (
                antenas_mais_proximas[i][0], 
                (lat_atual + (i * 0.00003), lon_atual + (i * offset), antenas_mais_proximas[i][1][2], antenas_mais_proximas[i][1][3], antenas_mais_proximas[i][1][4], antenas_mais_proximas[i][1][5], antenas_mais_proximas[i][1][6])
            )
    ...
    if ver_exist("UC_LAT_LONG", "UC", numero_uc) == True:
        ...
        if latitude_uc is not None and longitude_uc is not None:
            ...
            if antenas_proximas:
                ...
                cores_operadoras = {'TIM': 'blue', 'Claro': 'red', 'VIVO': 'purple', 'Sercomtel Ligga': 'orange'}

                for antena in antenas_proximas:
                    operadora = antena[2]
                    estacao = antena[4]
                    tecnologia = antena[5] if antena[5] else "Não disponível"
                    faixa = antena[6] if antena[6] else "Não disponível"
                    lat_antena = antena[0]
                    lon_antena = antena[1]
                    distancia_km = antena[3]
                    ...
                    popup_text = (
                        ...
                        f"Latitude: {float(lat_antena):.6f}\n"
                        f"Longitude: {float(lon_antena):.6f}\n"
                        f"Tecnologia: {tecnologia}\n\n"
                        f"Faixa: {faixa}"
                    )
                    folium.Marker((lat_antena, lon_antena), popup=popup_text, icon=folium.Icon(color=cor_icone)).add_to(parana_map)
